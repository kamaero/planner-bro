...
## [0.27] — 2026-06-03 — Scope доклада, слои проектов и PPTX-отчёт

### Добавлено
- **Scope доклада по ИТ-портфелю** — у проектов появились управленческие поля `project_kind`, `report_visibility`, `report_track`; они отделяют крупные проекты, ЦК/аутсорсинг, инициативы, планы отделов и служебные inbox-проекты.
- **Слои на дэшборде** — главный список теперь показывает крупные проекты отдельно; ЦК/аутсорсинг, инициативы и планы отделов вынесены в отдельные компактные блоки.
- **Scope-чекбокс проекта** — в создании и редактировании проекта добавлен простой чекбокс «Включать в scope доклада» без показа технических слоёв пользователю.
- **Регламентный PPTX-доклад** — генерация презентации сохранена на текущем шаблонном методе, но структура доклада теперь соответствует управленческому формату: обзорная инфографика, крупные проекты, ЦК/аутсорсинг, риски, инициативы и решения.
- **Обзорная инфографика доклада** — первый рабочий слайд получил KPI-карточки по проектам/задачам, диаграммы статусов и приоритетов, тепловую карту активности и блок контрольных сигналов.

### Изменено
- **ЦК 1С и ЦК INFOR LN** — вынесены в отдельный слой мониторинга ЦК/аутсорсинга; `ОРПС 1С` переименован в `ЦК 1С`, `План ЦК ИНФОР` переименован в `ЦК INFOR LN`.
- **Инициативы AI** — включены в scope доклада как портфель инициатив.
- **План ППО** — зафиксирован как крупный проект основного докладового контура.
- **Планы и служебные контейнеры** — `План ОАСУП`, `Отчётность`, `Срочные задачи (вне проектов)` скрыты из основного scope доклада.
- **Экспорты отчётов** — CSV/JSON/Slides JSON теперь получают тот же докладовый контур, что и PPTX.

### Исправлено
- **Диалог «Отделы» больше не управляет докладом** — из модалки назначения отделов убраны тип проекта, слой и включение в доклад, чтобы не путать пользователей.

---

## [0.26] — 2026-05-07 — Печатная форма ППО + права доступа

### Добавлено
- **ФИО согласующих в печатной форме** — подвал отчёта «Принт» дополнен фамилиями: Рахимов А.Ф., Подоприхин В.Н., Ульянова Л.М., Целоусова Н.С.
- **Выравнивание подписантов** — строки подписей свёрстаны через flexbox: должность слева, линия подписи по центру, фамилия справа

### Исправлено
- **Права на создание задач** — Подоприхин В.Н. добавлен в участники проекта «План ППО», исправлена ошибка «Недостаточно прав для этого действия»
- **Доступ к проектам** — Подоприхину ограничена видимость только проектом «План ППО»
- **Выравнивание шапки отчёта** — заголовки столбцов в печатной форме теперь по центру
