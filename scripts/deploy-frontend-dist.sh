...
REMOTE_HOST="${REMOTE_HOST:-planner_bro}"
...
npm run build:prod
chmod -R a+rX "$LOCAL_ROOT/frontend/dist"
...
ssh "$REMOTE_HOST" "chmod -R a+rX '$REMOTE_DIR/frontend/dist' && cd '$REMOTE_DIR' && docker compose -f docker-compose.prod.yml up -d frontend"
