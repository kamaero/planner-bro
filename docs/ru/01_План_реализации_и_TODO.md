...
1. Создать документ `Source of Truth`.
2. Создать документ `Code Map`.
3. Довести backend checks до полностью бесшовного запуска без ручной подготовки окружения.
4. Продолжить разрезание `projects.py` после первой вынесенной группы helper-правил.
5. Подготовить первую итерацию рефакторинга `frontend/src/pages/ProjectDetail.tsx`.
...
- Backend-проверки не являются однокнопочными в текущем окружении: нужен описанный и воспроизводимый способ запуска.
