...
- `backend/app/services/task_access_service.py` (access guards + title-only/scoped-rename fallback + read-visibility helper)
...
- `backend/app/services/task_activity_service.py` (assignee/status events + notification orchestration, включая `notify_task_created`)
...
- `backend/app/services/project_ai_draft_service.py` (approve use-case + AI draft/job query/reject helpers)
- `frontend/src/pages/ProjectDetail.tsx`
